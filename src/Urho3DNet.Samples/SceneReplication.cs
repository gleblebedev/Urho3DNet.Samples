...
namespace Urho3DNet.Samples
{
    ...
    public class SceneReplication : Sample
    {
        ...
        private const uint CTRL_LEFT = 4;
        private const uint CTRL_RIGHT = 8;

        // UDP port we will use
        private static readonly ushort SERVER_PORT = 2345;

        // Identifier for our custom remote event we use to tell the client which object they control
        private static readonly StringHash E_CLIENTOBJECTID = new StringHash("ClientObjectID");

        // Identifier for the node ID parameter in the event data
        private static readonly StringHash P_ID = new StringHash("ID");
        ...
        private readonly Timer packetCounterTimer_ = new Timer();

        private uint clientObjectID_;
        ...
        private void CreateScene()
        {
            ...
            var zoneNode = Scene.CreateChild("Zone", CreateMode.Local);
            ...
            CameraNode.Position = new Vector3(0.0f, 5.0f);
        }
        ...
        private void UpdateButtons()
        {
            ...
            var serverConnection = network.ServerConnection;
        ...
        private Node CreateControllableObject()
        {
            ...
            light.Range = 3.0f;
            light.Color = new Color(0.5f + ((uint) MathDefs.Rand() & 1u) * 0.5f,
                0.5f + ((uint) MathDefs.Rand() & 1u) * 0.5f, 0.5f + ((uint) MathDefs.Rand() & 1u) * 0.5f);
        ...
        private void HandlePhysicsPreStep(StringHash eventType, VariantMap eventData)
        {
        ...
        {
            var network = GetSubsystem<Network>();
            var address = textEdit_.Text.Trim();
        ...
        private void HandleDisconnect(StringHash eventType, VariantMap eventData)
        {
            var network = GetSubsystem<Network>();
            var serverConnection = network.ServerConnection;
        ...
        private void HandleConnectionStatus(StringHash eventType, VariantMap eventData)
        {
            UpdateButtons();
        }
...
// Then create a controllable object for that client
            ...
            var remoteEventData = new VariantMap();
        ...
        private void HandleClientDisconnected(StringHash eventType, VariantMap eventData)
        {
            ...
            var @object = serverObjects_[connection];
            if (@object != null)
                @object.Remove();
        ...
        private void HandleClientObjectID(StringHash eventType, VariantMap eventData)
        {
